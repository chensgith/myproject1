...
def compute_dice(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None):
    ...
    dices = np.zeros((len(png_name_list), num_classes))
    ...
    for ind in range(len(gt_imgs)):
        ...
        for i in range(num_classes):
            intersection = np.logical_and(label == i, pred == i)
            pred_i = pred[pred == i].size
            label_i = label[label == i].size

            if (pred_i + label_i) == 0:
                dice = np.nan  # 如果分母为零，设置为NaN
            else:
                dice = 2 * intersection.sum() / (label_i + pred_i)

            dices[ind, i] = dice

    mean_dice = np.nanmean(dices, axis=0)
    if name_classes is not None:
        for ind_class in range(num_classes):
            print('===>' + name_classes[ind_class] + ':\tDice-' + str(round(mean_dice[ind_class] * 100, 2)))

    print('===> Mean Dice: ' + str(round(np.nanmean(mean_dice) * 100, 2)))
